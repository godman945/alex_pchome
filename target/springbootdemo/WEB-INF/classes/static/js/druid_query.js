...
var defaultTitleContent= '<span id="titleSpan">造訪過的網頁必須與這個群組中的「任何」規則相符</span>';
...
function changeQueryGroupSelect(){
	if($("#queryGroupSelect").val() == 'query_any_group'){
		...
		$(".title_span").each(function() {
			this.innerHTML = '且與這個群組中的「任何」規則相符';
		});
	}else{
		$("#titleSpan")[0].innerHTML = "造訪過的網頁必須與這個群組中的「所有」規則相符";
		$(".title_span").each(function() {
			this.innerHTML = '且與這個群組中的「所有」規則相符';
		});
	}
}
...
function clickConditionButton(){
	...
	if($("#queryGroupSelect").val() == 'query_any_group'){
		...
		spanTitleContent = '<span class="title_span">且與這個群組中的「所有」規則相符</span>';
	}
	
	$("#conditionUl").append('<ul style="background-color:'+defaultColor+'">'+spanTitleContent+liStr+'<a href="javascript:void(0);" onclick="clickAddCondition(this);">且</a><a href="javascript:void(0);" onclick="deleteAllCondition(this);" style="float:right;margin-right:83%;">刪除此群組條件</a>'+'</ul>');
	if(defaultColor == 'aliceblue'){
		defaultColor = 'cornsilk';
	}else{
		defaultColor = 'aliceblue';
